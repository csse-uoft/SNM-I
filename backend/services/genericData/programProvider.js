const {GDBProgramProviderModel} = require("../../models");
...
const createSingleProgramProvider = async (req, res, next) => {
  const {providerType, data} = req.body;
  if (!providerType || !data)
    return res.status(400).json({message: 'Type or data is not given'});
  try {
    const provider = await GDBProgramProviderModel({type: providerType});
...
const fetchMultipleProgramProviders = async (req, res, next) => {
  try {
    const data = await GDBProgramProviderModel.find({},
      {
...
const updateProgramProvider = async (req, res, next) => {
...
const getProviderById = async (providerId) => {
  ...
  const provider = await GDBProgramProviderModel.findOne({_id: providerId},
    {
...
const fetchSingleProgramProvider = async (req, res, next) => {
  const {id} = req.params;
  try {
    const provider = await getProviderById(id);
    const providerType = provider.type;
...
const deleteSingleProgramProvider = async (req, res, next) => {
  const {id} = req.params;
  try {
    const provider = await getProviderById(id);
    const providerType = provider.type;
    ...
    // delete the provider
    await GDBProgramProviderModel.findByIdAndDelete(id);
    return res.status(200).json({success: true});
...
module.exports = {
  createSingleProgramProvider, fetchMultipleProgramProviders, fetchSingleProgramProvider, deleteSingleProgramProvider,
