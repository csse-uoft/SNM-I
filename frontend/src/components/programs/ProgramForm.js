...
import {
  ...
  ProgramSharedWithOptions, ProgramTypeOptions
} from '../../store/defaults'
...
import { createProgram, updateProgram } from '../../api/mockedApi/Programs'
...
function ProgramForm() {
  ...
  const [form, setForm] = useState(
    {
      ...
      current_capacity: Program.current_capacity || 0,
      frequency: Program.frequency || '',
      billable: Program.billable || 'No',
      price: Program.price || '',
      method_of_delivery: Program.method_of_delivery || '',
      method_of_registration: Program.method_of_registration || '',
      registration: Program.registration || '',
      email: Program.email || '',
      primary_phone_number: Program.primary_phone_number || '',
      alt_phone_number: Program.alt_phone_number || '',
      ...
      location_same_as_provider: Program.location_same_as_provider || false,
      share_with: Program.share_with || [],
      notes: Program.notes || '',
      ...
      }, Program.eligibility_conditions),
      provider_id: (Program.provider && Program.provider.id) || '',
      is_public: Program.is_public || false,
      /**
       * @type {[{condition_clauses: [field_name, clauses: [{left_operand, right_operand, operator}]]}]}
       **/
      eligibility_criteria: transformToFrontendType(Program.eligibility_criteria) || []
    }
  );
  ...
  useEffect(() => {
    Promise.all([
      fetchOntologyCategories('Programs').then(setProgramCategory),
  ...
  const submit = () => {
    ...
    if (validateForm()) {
      if (mode === 'edit') {
        updateProgram(id, form, (status, err, ProgramId) => {
        ...
        createProgram(form, (status, err, ProgramId) => {
          this.props.navigate('/Programs');
          this.forceUpdate()
        });
      }
  ...
  const formTitle = (mode === 'edit') ?
    'Edit Program Profile' : 'New Program';
  ...
  return (
    <Container className={classes.root}>
      ...
      {form.location_same_as_provider && form.provider_id &&
        <div>
          <RadioField
            label="Select the location this Program is provided at"
            value={form.location}
            options={locations}
            onChange={(e) => {
          ...
          {!_.includes(_.map(provider_locations, 'id'), form.location.id) &&
            <Typography style={{marginTop: 10}}>
              {`The current location of Program is at
      ...
      <MultiSelectField
        label="Share with"
        options={ProgramSharedWithOptions}
...
const mapStateToProps = (state) => {
  return {
    ...
    ProgramsCategories: state.ontology.Programs.categories,
    categoriesLoaded: state.ontology.Programs.loaded,
