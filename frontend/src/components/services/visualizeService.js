 ...
 * @returns {JSX.Element}
 */
export default function VisualizeService() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      getDynamicFormsByFormType('service').then(({forms}) => {
        setAllForms(forms);
      ...
      if (id) {
        // setForm
        const {data: genericData} = await fetchSingleGeneric('service', id);
    ...
    <Container>
      ...
      <Paper
        ...
        <Typography
          ...
          {'Information for Service with ID: ' + id}
        </Typography>
